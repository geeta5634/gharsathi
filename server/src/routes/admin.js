...
router.get('/users', authenticate, requireRole('admin'), async (req, res) => {
  try {
    const users = await query('SELECT id, name, phone, email, role, location, avatar, created_at FROM users ORDER BY created_at DESC');
    for (const u of users) {
      if (u.role === 'worker') {
        const worker = await queryOne('SELECT id, service_id, experience, rating, available FROM workers WHERE user_id = ?', u.id);
        u.worker = worker;
      }
    }
    res.json(users);
...
router.get('/users/customers', authenticate, requireRole('admin'), async (req, res) => {
  try {
    const customers = await query("SELECT id, name, phone, email, location, avatar, created_at FROM users WHERE role = 'customer' ORDER BY created_at DESC");
    for (const c of customers) {
      const bookingCount = await queryOne("SELECT COUNT(*) as c FROM bookings WHERE customer_id = ?", c.id);
...
router.post('/workers', authenticate, requireRole('admin'), async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      await execute('INSERT INTO users (id, name, phone, password, role, avatar) VALUES (?, ?, ?, ?, ?, ?)',
        id, sanitizeHtml(nameCheck.value), phoneCheck.value, hashed, 'worker', avatar);
      user = await queryOne('SELECT * FROM users WHERE id = ?', id);
    }

    const existing = await queryOne('SELECT id FROM workers WHERE user_id = ?', user.id);
    ...
    if (skills && Array.isArray(skills)) {
      for (const skill of skills) {
        if (typeof skill === 'string' && skill.trim()) {
          await execute('INSERT INTO worker_skills (worker_id, skill) VALUES (?, ?)', wid, skill.trim());
        }
      }
    }

    const worker = await queryOne('SELECT w.*, u.name, u.phone, u.avatar, s.name as service_name FROM workers w JOIN users u ON w.user_id = u.id JOIN services s ON w.service_id = s.id WHERE w.id = ?', wid);
...
router.get('/export/:type', authenticate, requireRole('admin'), async (req, res) => {
  try {
    ...
    if (type === 'workers') {
      data = await query('SELECT w.id, u.name, u.phone, u.email, s.name as service, w.experience, w.rating, w.reviews_count, w.visit_charge, w.available, w.earnings_total, w.trust_score FROM workers w JOIN users u ON w.user_id = u.id JOIN services s ON w.service_id = s.id');
