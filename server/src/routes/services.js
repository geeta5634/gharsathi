...
router.get('/', async (req, res) => {
  try {
    res.json(await query('SELECT * FROM services ORDER BY name'));
  } catch (err) {
...
router.post('/', authenticate, requireRole('admin'), async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await execute('INSERT INTO services (id, name, icon, color, description) VALUES (?, ?, ?, ?, ?)',
      id, name.trim(), icon.trim(), color.trim(), description ? description.trim() : null);
    res.status(201).json(await queryOne('SELECT * FROM services WHERE id = ?', id));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.put('/:id', authenticate, requireRole('admin'), async (req, res) => {
  try {
    ...
    const { name, icon, color, description } = req.body;
    await execute('UPDATE services SET name = ?, icon = ?, color = ?, description = ? WHERE id = ?',
